(()=>{
  ...
  const PROVIDERS=[
    ...
    research('jackall-official-research','JACKALL','JACKALL official-site factual-spec research only. Production publication remains disabled until rights are approved.'),
  ...
  const assertPublishable=(provider,licenseStatus)=>{if(!canPublish(provider,licenseStatus))throw new Error(`Catalog provider ${provider?.id||'unknown'} is not production-publishable`);return true};
