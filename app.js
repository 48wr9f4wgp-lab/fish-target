...
'テンヤ釣り':[
{name:'テンヤ＋キビナゴ',size:'3〜8号',color:'グロー',range:'表層〜中層',action:'スロー巻き＋誘い',when:'基準'},
...
function dynamicSize(base){const s=base||'基準サイズ';const rough=LIVE.marine&&((+LIVE.marine.wave||0)>=1.2||(+LIVE.marine.current||0)>=2);if((state.wind==='強い'||rough)&&/[g号]/.test(s))return s+'（上限寄り）';if(state.wind==='弱い'&&!rough&&/[g号]/.test(s))return s+'（軽めも可）';return s}
...
function rotationFor(p){const list=ROTATIONS[p.method]||[{name:p.bait||'基準',size:p.size||'基準',color:p.color||'基準色',range:p.range||'状況次第',action:p.baitAction||p.action||'基本操作',when:'基準'}];return list}
...
if($('autoReset'))$('autoReset').onclick=()=>{if(!cur)return;state.rotationManual=false;state.rotation=autoRotationIndex(basePlan());renderResult();toast('FIRST CASTをAUTOに戻した')};
renderFieldLive();
renderFilters();
