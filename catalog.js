(()=>{
  ...
  function productId({maker,category,series,generation='unknown',model}){return [maker,category,series,generation,model].map(slug).join(':')}
  function providerPublishable(product){const provider=providers?.byMaker?.(product?.maker)||null;return Boolean(provider&&providers.canPublish(provider,product?.source?.license_status))}
  function validateProduct(product,{production=false}={}){
    ...
    for(const key of numeric){if(specs[key]!==undefined&&specs[key]!==null&&finite(specs[key])===null)errors.push(`invalid numeric spec: ${key}`);if(finite(specs[key])!==null&&finite(specs[key])<0)errors.push(`negative numeric spec: ${key}`)}
    ...
    const expected=productId(product);if(product.product_id!==expected)errors.push('unstable product_id');
    return errors;
  }
  function validateCatalog(items,{production=false}={}){
    const errors=[],seen=new Set(),seenJan=new Map(),seenUpc=new Map();
    for(const product of items||[]){const local=validateProduct(product,{production});if(seen.has(product?.product_id))local.push('duplicate product_id');seen.add(product?.product_id);const jan=text(product?.identifiers?.jan),upc=text(product?.identifiers?.upc);if(jan){if(seenJan.has(jan)&&seenJan.get(jan)!==product?.product_id)local.push('duplicate JAN');else seenJan.set(jan,product?.product_id)}if(upc){if(seenUpc.has(upc)&&seenUpc.get(upc)!==product?.product_id)local.push('duplicate UPC');else seenUpc.set(upc,product?.product_id)}if(local.length)errors.push({product_id:product?.product_id||null,errors:local})}return errors;
  }
  const PRODUCTS=INPUT_ROWS.map(raw=>Object.freeze({...raw,product_id:raw.product_id||productId(raw),specs:Object.freeze({...raw.specs}),source:Object.freeze({...raw.source}),identifiers:Object.freeze({...raw.identifiers})}));
  function matches(product,{maker,category,series,status,statuses,query}={}){if(maker&&product.maker!==maker)return false;if(category&&product.category!==category)return false;if(series&&product.series!==series)return false;if(status&&product.status!==status)return false;if(Array.isArray(statuses)&&statuses.length&&!statuses.includes(product.status))return false;const q=queryText(query),ids=Object.values(product.identifiers||{}).join(' ');if(q&&!queryText(`${product.maker} ${product.series} ${product.model} ${product.display_name} ${ids}`).includes(q))return false;return true}
  function list(criteria={}){return PRODUCTS.filter(product=>matches(product,criteria))}
  function search(criteria={}){const offset=clampInt(criteria.offset,0,0,1000000),limit=clampInt(criteria.limit,50,1,100),matchesAll=list(criteria),items=matchesAll.slice(offset,offset+limit);return Object.freeze({items,total:matchesAll.length,offset,limit,hasMore:offset+items.length<matchesAll.length})}
  async function loadPage(criteria={}){return search(criteria)}
  function catalogIndex({category}={}){const rows=category?PRODUCTS.filter(p=>p.category===category):PRODUCTS;const makerEntries=[...new Set(rows.map(p=>p.maker))].map(maker=>{const makerRows=rows.filter(p=>p.maker===maker);return Object.freeze({maker,count:makerRows.length,series:[...new Set(makerRows.map(p=>p.series))]})});return Object.freeze({total:rows.length,makers:Object.freeze(makerEntries)})}
