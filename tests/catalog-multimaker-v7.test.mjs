...
const c=ctx.FISH_TARGET_CATALOG,g=c.list({maker:'GAMAKATSU'}).filter(x=>x.source.source_type==='manufacturer_official');

test('Gamakatsu keeps its 52-row official contract',()=>{assert.equal(g.length,52);assert.ok(c.makers.includes('GAMAKATSU'));assert.ok(g.every(x=>x.category==='rod'&&x.status==='current'));assert.ok(g.every(x=>x.source.source_provider==='gamakatsu-official-research'));assert.ok(g.every(x=>x.source.license_status==='restricted'&&!c.productionEligible(x)));const j=g.map(x=>x.identifiers.jan);assert.equal(new Set(j).size,52);assert.ok(j.every(x=>/^4549018\d{6}$/.test(x)))});
