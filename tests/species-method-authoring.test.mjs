...
const valid=()=>({
  ...
  targets:[{species_id:'species-test-target',name:'テスト魚',aliases:['試験魚'],water:'salt',shape:'small',tags:['テスト'],difficulty:'初級',season:{春:'春の条件',夏:'夏の条件',秋:'秋の条件',冬:'冬の条件'},default_method:method(null),methods:[method('alt_lure')]}],
...
test('valid authoring payload passes and flattens deterministically',()=>{
  ...
  assert.equal(runtime.targets[0].shape,'small');
  assert.equal(runtime.targets[0].season.秋,'秋の条件');
...
test('species visual and all four seasons are hard gates',()=>{
  const data=valid();
  data.targets[0].shape='';
  data.targets[0].season.冬='';
  const errors=validateAuthoring(data);
  assert.ok(errors.some(error=>error.includes('.shape')));
  assert.ok(errors.some(error=>error.includes('.season.冬')));
});
