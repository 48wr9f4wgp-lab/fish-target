...
const c=ctx.FISH_TARGET_CATALOG,rows=c.list({maker:'SHIMANO',series:'SURF LEADER'}).filter(x=>x.source.source_type==='manufacturer_official');

test('SURF LEADER keeps all 3 current official rows production-blocked',()=>{const batch=manifest.batches.find(x=>x.id==='shimano-surf-leader-2026');assert.ok(batch);assert.equal(batch.expected_rows,3);assert.equal(rows.length,3);assert.ok(rows.every(x=>x.category==='reel'&&x.status==='current'));assert.ok(rows.every(x=>x.source.source_provider==='shimano-official-research'&&x.source.license_status==='restricted'&&!c.productionEligible(x)));assert.equal(c.validateCatalog(c.products).length,0)});
