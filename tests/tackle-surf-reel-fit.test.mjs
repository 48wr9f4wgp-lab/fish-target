...
test('true casting plans prefer dedicated surf reels while retaining a generic spinning fallback',()=>{const {reelFit,castingPreferenceIntent}=tackleLogic();const plan={method:'投げ釣り',rod:'投げ竿 3.6〜4.2m',reel:'3000〜4000番',line:'PE 0.8〜1.2号'};const surf={size:null,reelSizeRaw:'35',applicationRaw:'投げ・遠投',lineType:'PE',lineNo:1};const generic={size:4000,lineType:'PE',lineNo:1};assert.equal(castingPreferenceIntent(plan),true);assert.equal(reelFit(surf,plan).level,0);assert.equal(reelFit(generic,plan).level,1)});

test('ordinary spinning-size plans do not force dedicated surf classification',()=>{const {reelFit}=tackleLogic();const plan={method:'ぶっ込み釣り',rod:'万能竿',reel:'3000〜5000番',line:'PE 0.8〜1.2号'};const surf={size:null,reelSizeRaw:'35',applicationRaw:'投げ・遠投',lineType:'PE',lineNo:1};const generic={size:4000,lineType:'PE',lineNo:1};assert.equal(reelFit(surf,plan).level,1);assert.equal(reelFit(generic,plan).level,0)});
