...
const c=ctx.FISH_TARGET_CATALOG,apia=c.list({maker:'APIA'}).filter(x=>x.source.source_type==='manufacturer_official'),initial=apia.filter(x=>!['GRANDAGE NAVAL','GRANDAGE WORLD OCEAN'].includes(x.series));

test('APIA initial batch keeps its 71 official rod contracts',()=>{const batch=manifest.batches.find(x=>x.id==='apia-rods-2026');assert.ok(batch);assert.equal(batch.expected_rows,71);assert.equal(initial.length,71);assert.equal(c.validateCatalog(c.products).length,0);assert.ok(initial.every(x=>x.category==='rod'));assert.ok(initial.every(x=>x.status==='unknown'));assert.ok(initial.every(x=>x.source.source_provider==='apia-official-research'));assert.ok(initial.every(x=>x.source.license_status==='restricted'&&!c.productionEligible(x)));const j=initial.map(x=>x.identifiers.jan);assert.equal(j.length,71);assert.equal(new Set(j).size,71);assert.ok(j.every(x=>/^\d{13}$/.test(x)))});
