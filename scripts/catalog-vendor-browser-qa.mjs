...
async function chooseRod(page,maker,series,query,expected){await page.locator('#rodCatalogMaker').selectOption({label:maker});await page.locator('#rodCatalogSeries').selectOption({label:series});await page.locator('#rodCatalogSearch').fill(query);const exact=({expected,series})=>[...document.querySelectorAll('#rodCatalogModel option')].find(o=>{const t=(o.textContent||'').trim();return t===expected||t.startsWith(`${expected} ·`)||t===`${series} · ${expected}`||t.startsWith(`${series} · ${expected} ·`)});await page.waitForFunction(exact,{expected,series},{timeout:15000});const value=await page.locator('#rodCatalogModel option').evaluateAll((opts,{expected,series})=>opts.find(o=>{const t=(o.textContent||'').trim();return t===expected||t.startsWith(`${expected} ·`)||t===`${series} · ${expected}`||t.startsWith(`${series} · ${expected} ·`)})?.value||null,{expected,series});assert.ok(value,`${series} ${expected}`);const model=page.locator('#rodCatalogModel');await model.selectOption(value);await model.dispatchEvent('change');return value}
try{
